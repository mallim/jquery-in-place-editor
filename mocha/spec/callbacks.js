describe( 'callbacks', function () {
  ...
  it( 'shoud call preinit callback on click', function ( done ) {
    var sensor = false;
    var originalText = this.sandbox.text();
    expect( this.sandbox.text() ).to.be.eq( originalText );
    this.openEditor({
      preinit: function(domNode){ sensor = domNode.clone(true); return ''; }
    });
    expect( sensor.text() ).to.be.eq( originalText );
    done();
  });

  it( 'should not open editor if preinit returns false', function ( done ) {
    this.openEditor({
        preinit:function(){
            return false;
        }
    });
    expect( this.sandbox.find( 'input' ) ).not.to.be.exist;
    done();
  });

  it( 'should open the editor if preinit returns undefined (i.e. nothing)', function ( done ) {
    this.openEditor({
        preinit:function(){
        }
    });
  ...
  it.skip( 'should not call preinit if element is cancelled', function( done ){
    ...
    expect( sensor ).to.be.eq( 'not called' );
    done();
  });

  it( 'should call postclose after editor is closed', function( done ){
    var sensor;
    var options = {
      postclose: function(domNode) {
        sensor = domNode.clone()
      }
    };
    this.edit( options, 'fnord' );
    expect( sensor.text() ).to.be.eq( "fnord" );
    console.log( sensor.children().html() );
    done();
  });
  ...
  describe 'uses dom element of editor as this for callbacks'
    ...
    it 'error_sink'
      this.edit({ error_sink: this.sensor, callback: -{} })
    end
